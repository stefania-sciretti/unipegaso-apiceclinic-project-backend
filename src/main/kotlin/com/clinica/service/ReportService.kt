...
class ReportService(
    ...
    fun create(request: ReportRequest): ReportResponse {
        ...
        if (appointment.status != AppointmentStatus.COMPLETED) {
            throw IllegalStateException("Cannot create a report for an appointment that is not COMPLETED")
        }
        if (reportRepository.existsByAppointmentId(request.appointmentId)) {
            throw IllegalStateException("A report already exists for appointment id: ${request.appointmentId}")
        }
